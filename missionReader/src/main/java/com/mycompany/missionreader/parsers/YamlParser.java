 ...
 private MissionBuilder builder;
    ...
    public Mission parse(File file) {
        try {
            ...
            if (jsonNode.has("missionId")) builder.setMissionId(jsonNode.get("missionId").asText());
            if (jsonNode.has("date")) builder.setDate(LocalDate.parse(jsonNode.get("date").asText()));
            if (jsonNode.has("location")) builder.setLocation(jsonNode.get("location").asText());
            if (jsonNode.has("outcome")) builder.setOutcome(MissionOutcome.fromStringToEnum(jsonNode.get("outcome").asText()));
            if (jsonNode.has("damageCost")) builder.setDamageCost(jsonNode.get("damageCost").asInt());
            ...
            if (jsonNode.has("sorcerers") && jsonNode.get("sorcerers").isArray()) {
                ...
                if (!sorcerers.isEmpty()) builder.setParticipants(sorcerers);
            }
            ...
            if (jsonNode.has("techniques") && jsonNode.get("techniques").isArray()) {
                ...
                if (!techniques.isEmpty()) builder.setTechniqueUsed(techniques);
            }
